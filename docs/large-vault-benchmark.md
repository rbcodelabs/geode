...
On vault startup, main prepares the WAL database before launching the utility and retains that connection for later cache operations. This orders first-open journal/schema bootstrap without awaiting utility reconciliation. A two-connection test demonstrates the captured SQLite journal-transition lock mechanism, not the exact original utility interleaving or the separate unavailable-indexer cause. Failed initialization or utility launch closes the prepared connection; subsequent read failures remain explicit cache misses, not retry loops or increased lock timeouts.
