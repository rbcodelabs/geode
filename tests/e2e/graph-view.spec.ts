...
test("opens the graph view, builds nodes/edges from the vault, and click-to-opens a note", async () => {
  ...
  try {
    ...
    await expect(window.locator(".workspace-tab-title")).toHaveCount(2);
    await window.keyboard.press(isMac ? "Meta+P" : "Control+P");
    await window.locator(".prompt-input").fill("Graph view");
    await window.getByText("Graph view: Open graph view").click();
    await expect(window.locator(".workspace-tab-title")).toHaveCount(2); // still 2, not 3
    await expect(window.locator(".workspace-tab-title:text-is('Graph view')")).toHaveCount(1);
    await expect(window.locator(".workspace-tab-header.is-active .workspace-tab-title")).toHaveText(
      "Graph view"
    );
    ...
    await expect(window.locator(".workspace-tab-title")).toHaveCount(2); // the other tab is untouched
