...
test("a Geode plugin provides full-vault sync through the built-in approval UI", async () => {
    ...
    module.exports.default = class extends Plugin {
      onload() {
        this.registerSyncProvider({
          ...
          open: async () => ({
            scan: async () => ({ status: 'complete', mode: 'snapshot', cursor: 'first', entries: [{ id: 'r1', path: 'Remote.md', kind: 'file', revision: '1', size: 6 }] }),
  ...
  try {
    ...
    await expect(window.locator(".notice")).toContainText('"uploads":1');
    await expect(window.locator(".notice")).toContainText('"downloads":1');
