...
export function createLegacyGeodeFacade(host: HostServices): GeodeApi {
  ...
  return {
    ...
    claimSecretCapability: async owner => owner,
    readSecret: (owner, key) => host.secrets.forOwner(owner).get(key),
    writeSecret: (owner, key, value) => host.secrets.forOwner(owner).set(key, value),
    removeSecret: (owner, key) => host.secrets.forOwner(owner).remove(key),
