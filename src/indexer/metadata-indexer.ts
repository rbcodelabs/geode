...
/**
 * The metadata body-scan cap now belongs to the portable engine
 * (`src/wiki/constants.ts`): it describes the file format the parser honours,
 * not this indexer's snapshot/reconcile machinery, and the portable parser
 * needs it without taking a dependency on anything under `src/indexer/`.
 *
 * Re-exported here so every existing import site — renderer settings, the main
 * process, the indexer utility process — keeps working against one definition.
 * See `docs/design/headless-phase0.md`'s extraction dependency map.
 */
export {
